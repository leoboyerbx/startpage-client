<template>
  <article class="bg-white shadow-th-sm rounded-xl w-full overflow-hidden">
    <header class="py-3 px-4 text-gray-700 shadow">
    ...
    <main :style="{ maxHeight: maxHeight + 'px' }" class="overflow-y-scroll no-scrollbar transition-all duration-300" ref="listWrapper">
      <div ref="list">
        <a
          ...
          :href="link.URL"
          target="_blank"
          class="flex items-center p-2 text-gray-800"
          :title="link.title"
        >
    ...
    <footer @click.prevent="extended = !extended" v-if="category.links.length > 5" class="border-t border-gray-200 p-2 flex justify-center items-center text-gray-600 cursor-pointer">
